    ...
    <div className="auth-bg min-h-dvh flex items-center justify-center px-4 py-12">
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <motion.div
          ...
          className="absolute -top-40 -right-40 h-96 w-96 rounded-full bg-orange-500/10 blur-3xl"
        />
        <motion.div
          animate={{ scale: [1, 1.2, 1], opacity: [0.5, 0.9, 0.5] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }}
          className="absolute -bottom-40 -left-40 h-96 w-96 rounded-full bg-violet-600/10 blur-3xl"
        />
      </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <motion.div {...fadeUp(0.1)} className="glow-card p-8">
          <h1 className="text-2xl font-extrabold tracking-tight text-white">Create an account</h1>
          <p className="mt-1 text-sm text-white/50">Join thousands of learners today.</p>
          ...
          <form className="mt-7 space-y-5" onSubmit={onSubmit}>
            ...
            <motion.div {...fadeUp(0.34)}>
              <label className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-white/40">Confirm password</label>
              <input
                type="password"
                value={confirm}
            ...
            <motion.div {...fadeUp(0.4)}>
              <button
                ...
                className="flex items-center gap-2 text-xs font-bold text-white/30 hover:text-orange-400 transition"
              >
                <span className="text-base leading-none">{showAdminField ? "▾" : "▸"}</span>
              ...
              {showAdminField && (
                <motion.div
                  ...
                  className="mt-3"
                >
                  <label className="mb-1.5 block text-xs font-bold uppercase tracking-widest text-orange-400/70">Admin secret key</label>
                  <input
                    type="password"
                    value={adminSecret}
                    onChange={(e) => setAdminSecret(e.target.value)}
                    placeholder="Enter admin secret"
                    className="input-glow w-full rounded-xl border border-orange-500/30 bg-orange-500/5 px-4 py-3 text-sm text-white placeholder:text-white/25 transition"
                  />
                  <p className="mt-1.5 text-xs text-white/30">Leave blank to register as a student.</p>
                </motion.div>
              )}
            </motion.div>
          ...
          <motion.div {...fadeUp(0.52)} className="mt-6 text-center text-sm text-white/40">
            Already have an account?{" "}
            <Link className="font-bold text-orange-400 hover:text-orange-300" to="/login">Sign in</Link>
          </motion.div>
        </motion.div>

        <motion.p {...fadeUp(0.56)} className="mt-6 text-center text-xs text-white/20">
